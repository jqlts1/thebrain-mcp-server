...
@mcp.tool()
def search_thoughts(query: str, n: int = 30):
...
@mcp.tool()
def get_graph(thought_id: str, siblings: bool = False):
    """获取想法的图谱关系（子、父、跳转等）"""
    return client.get_graph(thought_id, siblings)

@mcp.tool()
def create_thought(name: str, parent_id: str = None, jump_id: str = None, kind: int = 1):
...
@mcp.tool()
def update_thought(thought_id: str, name: str = None, label: str = None, color: str = None, type_id: str = None):
...
@mcp.tool()
def manage_link(thought_id_a: str, thought_id_b: str, relation: int = 3, name: str = None, action: str = "create", link_id: str = None):
...
@mcp.tool()
def operate_note(thought_id: str, content: str = None, append: bool = False, format: str = "markdown"):
    """操作想法的笔记。如果提供 content，则根据 append 参数设置或追加笔记。否则获取笔记。"""
    if content:
        if append:
...
@mcp.tool()
def list_metadata(category: str):
...
@mcp.prompt()
def summarize_thought(thought_id: str):
    """生成一个用于总结特定想法的提示词"""
